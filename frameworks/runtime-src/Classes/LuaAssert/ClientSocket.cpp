...
int CClientSocket::NewsendData(const char* pData)
{
	return NewSendSocketData(pData);
}
...
bool CClientSocket::NewSendSocketData(const char* pData)
{
	if (pISocketServer)
		return pISocketServer->NewSendSocketData(pData);
	return false;
}
