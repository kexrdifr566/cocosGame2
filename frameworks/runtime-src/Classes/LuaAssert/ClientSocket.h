...
public:
	...
	virtual bool NewSendSocketData(const char* pdata);
...
public:
	...
	int NewsendData(const char* pData);
};
...
static int toLua_Client_Socket_NewsendData(lua_State* tolua_S)
{
	int result = 0;
	do
	{
		...
		int argc = lua_gettop(tolua_S) - 1;
		CC_BREAK_IF(argc != 1);
		const char* pData = lua_tostring(tolua_S, 2);
		//CCmd_Data* pData = (CCmd_Data*)tolua_tousertype(tolua_S, 2, 0);
		CC_BREAK_IF(pData == NULL);

		result = cobj->NewsendData(pData);
	} while (false);
...
static int register_all_client_socket()
{
	...
	tolua_function(tolua_S, "NsendData", toLua_Client_Socket_NewsendData);
