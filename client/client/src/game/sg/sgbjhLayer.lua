local sgbjhLayer = class("sgbjhLayer", function ()
	local sgbjhLayer =  display.newLayer()
	return sgbjhLayer
end)
...
function sgbjhLayer:ctor(_scene)
	...
	local rootLayer, csbNode = ExternalFun.loadRootCSB("Game/sgLayer/sgbjhLayer.csb", self)
...
function sgbjhLayer:changebj()
	local bjindex = cc.UserDefault:getInstance():getIntegerForKey("SGBJHBJ", 1)
	local str = string.format("Game/public/shezhi/bj%d.png",bjindex)
    
	self.bj:setBackGroundImage(str)
	self.Card:changebj()
end
function sgbjhLayer:reGame(bol) 				   -----------重置界面
    self:stopAllActions()
...
function sgbjhLayer:playerinint()
...
function sgbjhLayer:topinint(jushu)
	self.top:updateT(jushu)
end
function sgbjhLayer:onButtonClickedEvent(tag,ref)
	--print("操作"..tag )
    if tag > 10 and tag <=16  then  --玩家的操作。
    	...
    	local data ={}
		data.actionCode =SgbjhHead.action[1]
		data.QiangZhuangBs=bs
    	gst.send(SgbjhHead.Tcode,data)
    ...
    elseif tag >= 31 and tag <= 34 then   		--加注操作
    	...
    	local data ={}
    	data.actionCode =SgbjhHead.action[2]
    	data.betBs = add
    	gst.send(SgbjhHead.Tcode,data)
    elseif tag  == 50 then
    	local data ={}
...
function sgbjhLayer:addToRootLayer(node, zorder)
	if nil == node then
		return
	end
...
function sgbjhLayer:showBP(cuo,kai)                                           --显示搓牌OR开牌
        --搓牌
    self.BP[1]:setVisible(cuo)
    self.BP[2]:setVisible(kai)
end
function sgbjhLayer:showbeishu(data)                                          --显示抢庄
...
function sgbjhLayer:showxiazhu(data)                                          --显示下注
    for k,v in pairs(data) do
...
function sgbjhLayer:setHdVolume(baifenbi)
    -- do somethings
...
function sgbjhLayer:setHt(min,max)
...
function sgbjhLayer:getbankview()                                              --获取庄现在得位置
	return self.scene:getotherchair(self.banker)
end
function sgbjhLayer:openCard(isCuo)
    ...
    local callFuncEnd = function()
        ...
        local data ={}
    	data.actionCode =SgbjhHead.action[3]
    	gst.send(SgbjhHead.Tcode,data)
    end
    if isCuo then
...
--飞金币动画
function sgbjhLayer:showGoldToAreaWithID(beganviewid,endviewid)
	local pos = endviewid
...
function sgbjhLayer:KillGameClock()
...
function sgbjhLayer:SetGameClock(time,index)
    self:KillGameClock()
    local str=''
    if index == 1 then
        str="等待玩家叫庄"
...
function sgbjhLayer:OnClockUpdata()
...
---  继承关系--------- 座位
--获取自己的座位号
function  sgbjhLayer:getMechair()
	return self.scene:getMechair()
end
--用自己的座位号计算其他的人座位号
function  sgbjhLayer:getotherchair(index)
	return self.scene:getotherchair(index)
end
--用用户ID 来算出他的座位号
function sgbjhLayer:getchairindex(UserCode)
	return self.scene:getchairindex(UserCode)
end
--------------------------------------------------------------消息层-----------------------------------------------------
--显示金币飞到庄家处
function sgbjhLayer:showGoldTo(Tchair,coumalist)
...
--获取随机显示位置
function sgbjhLayer:getRandPos(nodeArea)
...
-- function sgbjhLayer:getMoveAction(beginpos, endpos, callback, rtime, bScale, inorout, isreverse)
...
-- end
function sgbjhLayer:getPlayerGetPos(Tchair)
    local x,y=0
    x,y =self.player.P[Tchair]:getPosition()
    return cc.p(x,y)
end
function sgbjhLayer:message(code,data)
--dump(data,"三公界面消息："..code)
	if code == GameHead.ingame  then   						--进入房间消息
            ...
            for k,v in pairs(data.users) do
                ...
                if v.cards and #v.cards~= 0  then
                    if  self.gameStatus == "5"  then
                        self.Card:showCard(otherindex,v.cards,v.pxList)
        ...
        if ( self.gameStatus == "25" or self.gameStatus == "26" or self.gameStatus == "5")   then  --and data.userHasBetAll
	...
	elseif code == GameHead.zkts then                     							--提示庄开启游戏按钮
    	self.B_k:setVisible(true)
    elseif code == SgbjhHead.bgame then                     							--开始游戏界面
    ...
    elseif code == SgbjhHead.tqiang  then          --抢庄结果
    ...
    elseif code == SgbjhHead.tqiangres    then             --叫庄的最后结果
        self:KillGameClock()
        self:noShowBB()                                  --隐藏叫庄按钮 
    ...
    elseif code ==SgbjhHead.tcodexz       then            --显示玩家下注
        if data.chairIndex == self.scene:getMechair() then
    ...
    elseif code ==SgbjhHead.tcodexzres       then            --显示玩家下注最终
        self:KillGameClock()
        self:noShowBB() 
        --local call = function()
            --要显示倒计时的地方
	...
	elseif code == SgbjhHead.smallend then 
        ...
        if data.bankerChair then
            ...
            for k,v in pairs(data.data) do 											--更新玩家手中的牌及牌型
                ...
                if v.coin <0 then
                    self.Card:showCard(chair,v.pxCards,v.pxList,Sex,false,v.coinList)
                else
                    self.Card:showCard(chair,v.pxCards,v.pxList,Sex,true,v.coinList)
                end
