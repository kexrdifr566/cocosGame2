local brlhdLayer = class("brlhdLayer", function ()
	local brlhdLayer =  display.newLayer()
	return brlhdLayer
end)
...
local beilv={2,2,7}
function brlhdLayer:ctor(_scene)

	self:playerinint()

	self.scene=_scene
	local rootLayer, csbNode = ExternalFun.loadRootCSB("Game/brLayer/brlhdLayer.csb", self)
    ...
    --动画
    self.GameAction=self.action:getChildByName("A")
...
function brlhdLayer:reGame(bol) 				   -----------重置界面
    self:stopAllActions()
 ...
 --    for i=1,9 do
 --        local bb=string.format("A%d",i)
 --        self.action:getChildByName(bb):setVisible(false)
	-- end
    self.GameAction:setVisible(false)
    --清理牌资源
...
function brlhdLayer:playerinint()
    ...
    self.zongzhulist={0,0,0}             --总住列表
    self.Metouzhulist={0,0,0}             --总住列表
    self.XutouChouma={0,0,0}             --续投筹码
    self.isXiazhu=false
    self.KeTouScore=0
    self.YiTouScore=0
    self.OtherTou={}
...
function brlhdLayer:SelectChouma(index)
    --筹码已经选中
...
function brlhdLayer:settingcouma(ble)
        --筹码不可用状态
    for k,v in pairs(self.CM) do
        self.CM[k]:setEnabled(ble)
    end
end
function brlhdLayer:onButtonClickedEvent(tag,ref)
	--print("操作"..tag )
    if tag == 1 then                    --退出游戏
...
function brlhdLayer:panduanXia(tag)
...
--下注
function brlhdLayer:pushBet(index)
     local quyu={}
        for i=1,3 do
            quyu[i]=0
        end
...
function brlhdLayer:addToRootLayer(node, zorder)
	if nil == node then
		return
	end
...
--设置牌背景
function brlhdLayer:setCardBack(isVisib)
    for k,v in pairs(self.Card) do
        v:removeAllChildren()
        v:setVisible(isVisib)
	end
end
function brlhdLayer:setXiaZhuQY(ble)
    for k,v in pairs(self.QY) do
        v:setEnabled(ble and ble or self.statusXia)
	end
end
function brlhdLayer:KillGameClock()
...
function brlhdLayer:SetGameClock(time,roomStatus)
    self:KillGameClock()
    local str=''
...
function brlhdLayer:OnClockUpdata(roomStatus)
...
-- function brlhdLayer:OnClockOtherTou()
...
function brlhdLayer:OtherTouzhu()
...
function brlhdLayer:killotherTou()
    self.OtherTou={}
...
--开牌
function brlhdLayer:GetOpenAction(card,cards)
	local duration =0.1
...
--开牌
function brlhdLayer:showCard(index,cards,paixing)			 		--显示完整牌
...
function brlhdLayer:showCardType(index,cardtype)
...
--设置庄和下庄按钮
function brlhdLayer:setZhuangBtn(isbanker,isstatus,isend)
...
function brlhdLayer:message(code,data)
    ...
    elseif code ==BaiRenHead.Gamestart then
        ...
        self.m_coumaList = {{}, {}, {}, {}}
        --更新区域总住等筹码 
        self.zongzhulist={0,0,0}              --总住
        self.XutouChouma=self.Metouzhulist
        self.Metouzhulist={0,0,0}             --续投
        ...
        --self:showGoldToAreaWithMe(data.winOrLoss,data.bankerWinOrLoss)
         ...
         performWithDelay(self,function () 
                ...
                --for i=1,4 do
                    --if winOrLoss == -1 then
                local index=0
                if data.winLossChair == -1 then
                    index=1
                    ExternalFun.playSoundEffect("long",true)
                elseif data.winLossChair == 0 then
                    index=3
                elseif data.winLossChair == 1 then
                    index =2
                    ExternalFun.playSoundEffect("hu",true)
                end
                table.insert(self.blinkPos,index)
                ...
                self:updateMyScore(self.Myscore)
                --更新记录
                self:gameHis(data.winLossChair)
         end,5.5-2.4)
        --测试用动画
        self:KillGameClock()
...
-------------------------------------------------------------消息相应层
function brlhdLayer:gameHis(data,ble)
    -- do somethings
    --
    ...
    if ble then
        ...
        local item=self.Trendp:clone()
        --设置头像
        item:getChildByName("t"):loadTexture(str[data+2])
        --如果是五倍得时候
        if index == 20 then
            for i=1,19 do
...
--飞金币动画
function brlhdLayer:showGoldToAreaWithID(beganviewid,endviewid)
	local pos = endviewid
...
function brlhdLayer:showcoumaMove()
    local x,y =self.p_t:getPosition()
    local bankerheadpoint=cc.p(x,y)
...
--判断位置
function brlhdLayer:getPlayerGetPos(userCode)
    local x,y=0
    --自己的位置 
...
function brlhdLayer:showEnd(node,changescore,ying,isZhuang,isYou)
...
function brlhdLayer:onSendPlaceJettonChip(tag,beginpos,betIdx,goldnum,ftime)
...
--获取随机显示位置
function brlhdLayer:getRandPos(nodeArea)
    if nodeArea ==self.QY[3] then
        local beginpos = cc.p(nodeArea:getPositionX() - 350, nodeArea:getPositionY() - 60)
        local offsetx = math.random()
        local offsety = math.random()
        return cc.p(beginpos.x + offsetx * 700, beginpos.y + offsety * 100)
    else
        local beginpos = cc.p(nodeArea:getPositionX() - 200, nodeArea:getPositionY() - 60)
        local offsetx = math.random()
        local offsety = math.random()
        return cc.p(beginpos.x + offsetx * 400, beginpos.y + offsety * 100)
    end
end
...
-- function brlhdLayer:getMoveAction(beginpos, endpos, callback, rtime, bScale, inorout, isreverse)
...
-- end
function brlhdLayer:PlayGameAction(atype)
    self.GameAction:setVisible(true)
    local anilist={"ani_lhd_ksxz","ani_lhd_tzxz","ani_sg_ts","ani_sg_tp"}
    local animation = self.GameAction:getAnimation()
    if atype == 1 or atype ==2 then
        animation=self.GameActionB:getAnimation()
    end
...
function brlhdLayer:QuyuScoreUpdate(isMe,RoundBet)
...
function brlhdLayer:UpdateQuYuScore()
...
function brlhdLayer:UpdateQuYuMeScore()
...
--更新庄家列表
function brlhdLayer:updateZhuanglist(data)
    --区分单人模式
    if self.brMs ==1 then
...
--更新富豪榜
function brlhdLayer:updateFuhaolist(data)
    local index =#data
    for i=1,6 do
        local bb=string.format("p%d",i)
...
--更新神算子
function brlhdLayer:updateSuanzilist(data)
    local index =#data
    for i=1,6 do
        local bb=string.format("p%d",i)
...
--更新自己的金币
function brlhdLayer:updateMyScore(score)
    if score then
