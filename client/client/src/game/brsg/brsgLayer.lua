local brsgLayer = class("brsgLayer", function ()
	local brsgLayer =  display.newLayer()
	return brsgLayer
end)
...
function brsgLayer:ctor(_scene)

	self:playerinint()

	self.scene=_scene
	local rootLayer, csbNode = ExternalFun.loadRootCSB("Game/brLayer/brsgLayer.csb", self)
...
function brsgLayer:changebj()
	local bjindex = cc.UserDefault:getInstance():getIntegerForKey("BRSGBJ", 1)
	local str = string.format("Game/brpublic/brbj%d.png",bjindex)
	self.bj:setBackGroundImage(str)
end
function brsgLayer:reGame(bol) 				   -----------重置界面
    self:stopAllActions()
...
function brsgLayer:playerinint()
...
function brsgLayer:SelectChouma(index)
    --筹码已经选中
...
function brsgLayer:settingcouma(ble)
        --筹码不可用状态
    for k,v in pairs(self.CM) do
        v:setEnabled(ble)
    end
end
function brsgLayer:onButtonClickedEvent(tag,ref)
	--print("操作"..tag )
    if tag == 1 then                    --退出游戏
    ...
    elseif tag == 6 then                --下庄按钮
    	...
    	gst.send(BaiRenHead.Tcode,data)
    elseif tag == 7 then
        local layer =BrRuleLayer:create(self,2)
        self:add(layer)
...
function brsgLayer:panduanXia(tag)
...
--下注
function brsgLayer:pushBet(index)
     local quyu={}
        for i=1,9 do
            quyu[i]=0
        end
...
function brsgLayer:addToRootLayer(node, zorder)
	if nil == node then
		return
	end
...
--设置牌背景
function brsgLayer:setCardBack(isVisib)
...
function brsgLayer:setXiaZhuQY(ble)
    for i=1,9 do
        self.QY[i]:setEnabled(ble and ble or self.statusXia)
	end
end
function brsgLayer:KillGameClock()
...
function brsgLayer:SetGameClock(time,roomStatus)
    self:KillGameClock()
    local str=''
...
function brsgLayer:OnClockUpdata(roomStatus)
...
function brsgLayer:OnClockOtherTou()
...
function brsgLayer:OtherTouzhu()
...
function brsgLayer:killotherTou()
    self.OtherTou={}
...
--开牌
function brsgLayer:GetOpenAction(card,cards)
	local duration =0.1
...
--开牌
function brsgLayer:showCard(index,cards,paixing)			 		--显示完整牌
...
function brsgLayer:showCardType(index,cardtype)
...
--设置庄和下庄按钮
function brsgLayer:setZhuangBtn(isbanker)
...
function brsgLayer:message(code,data)
...
function brsgLayer:UpdateTouAndsKe()
...
function brsgLayer:showGoldToAreaWithMe(winOrLoss,bankerWinOrLoss)
    local detime=3
...
--飞金币动画
function brsgLayer:showGoldToAreaWithID(beganviewid,endviewid)
	local pos = endviewid
...
function brsgLayer:showcoumaMove()
    local x,y =self.p_t:getPosition()
    local bankerheadpoint=cc.p(x,y)
...
--判断位置
function brsgLayer:getPlayerGetPos(userCode)
    local x,y=0
    --自己的位置 
...
function brsgLayer:showEnd(node,changescore,ying,isZhuang,isYou)
...
function brsgLayer:onSendPlaceJettonChip(tag,beginpos,betIdx,goldnum,ftime)
    local fun = function( ... )
		--if isme then
...
--获取随机显示位置
function brsgLayer:getRandPos(nodeArea)
...
-- function brsgLayer:getMoveAction(beginpos, endpos, callback, rtime, bScale, inorout, isreverse)
...
-- end
function brsgLayer:PlayGameTAction(atype)
    local str="animation"
    if atype ==4 then 
        str="animation2"
    end
...
function brsgLayer:QuyuScoreUpdate(isMe,RoundBet)
...
--通杀OR通配
function brsgLayer:PlayGameXiaZhuAction(atype)
    local str="animation"
    if atype ==2 then 
        str="animation2"
    end
...
function brsgLayer:UpdateQuYuScore()
...
function brsgLayer:UpdateQuYuMeScore()
...
--更新庄家列表
function brsgLayer:updateZhuanglist(data)
    self.p_t:removeAllChildren()
...
--更新富豪榜
function brsgLayer:updateFuhaolist(data)
    local index =#data
    for i=1,6 do
        local bb=string.format("p%d",i)
...
--更新神算子
function brsgLayer:updateSuanzilist(data)
    local index =#data
    for i=1,6 do
        local bb=string.format("p%d",i)
...
--更新自己的金币
function brsgLayer:updateMyScore(score)
    if score then
