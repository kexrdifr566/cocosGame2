...
function playerLayer:ctor(scene,number)
	...
	local rootLayer, csbNode = ExternalFun.loadzRootCSB("Game/dzLayer/PlayerLayer.csb", self)
	for i = 6, 10 do
...
function playerLayer:reGame(bol)
	...
	for k,v in pairs(self.P) do
		...
		v:getChildByName("m"):setVisible(false)	 --隐藏大盲注小盲注图标
...
function playerLayer:showMang(index,ble)
	local str=string.format("img_xiaomanzhu.png")
	if ble then
		str=string.format("img_damanzhu.png")
	end
	self.P[index]:getChildByName("m"):loadTexture(str,ccui.TextureResType.plistType)
	self.P[index]:getChildByName("m"):setVisible(true)
end
----------------------显示读秒
function playerLayer:showG(index,time)
	...
	-- local pp= cc.ProgressTimer:create(display.newSprite("Game/public/tubiao2.png"))
...
function playerLayer:showEnd(index,changescore,gameCoin,ying)
	...
	local call = cc.CallFunc:create(function( )
		--if not tolua.isnull(self.m_atlasScore) then
			...
			--self.num:removeFromParent()
	end)
	local moveBy = cc.MoveTo:create(0.4, cc.p(20, 75))
