...
function Bridge_ios.getMachineId()
    ...
    if not ok then
        ...
        local machineid = cc.UserDefault:getInstance():getStringForKey("machineid") 
		if machineid then
			return machineid
		end
		machineid =  os.time()+math.random( 1000,10000 )
		cc.UserDefault:getInstance():setStringForKey("machineid",machineid)
        return machineid --"A501164B366ECFC9E249163873094D50"
    else
...
--选择图片
function Bridge_ios.pickImg( callback, needClip )
...
function Bridge_ios.thirdPartyPay(thirdparty, payparamTab, callback)
    -- local payparams={}
    -- payparams.info = cjson.encode(payparamTab)
    payparamTab._nidx = thirdparty
    payparamTab.scriptHandler = callback
    --dump(payparamTab,"payparamTab>>>>")
    local ok,ret  = luaoc.callStaticMethod(BRIDGE_CLASS,"appleStorePay",payparamTab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return false, msg   
...
function Bridge_ios.getClipboard(callback)
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"getClipboard")
    if not ok then
...
function Bridge_ios.setPlaybackRate( data )
    -- local paramtab = data
    -- local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"openRTMP", paramtab)

    --  local arg1 = {urlString = rtmppath, stateDidChange = stateDidChange}
    -- local luaoc = require "cocos.cocos2d.luaoc"
    local ok,ret = luaoc.callStaticMethod("NativeOcClass", "setPlaybackRate", data)
...
function Bridge_ios.openRTMP( data )
    -- local paramtab = data
    -- local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"openRTMP", paramtab)

    --  local arg1 = {urlString = rtmppath, stateDidChange = stateDidChange}
    -- local luaoc = require "cocos.cocos2d.luaoc"
    local ok,ret = luaoc.callStaticMethod("NativeOcClass", "play", data)
...
function Bridge_ios.closeRTMP( )
    -- local paramtab = data
    local ok,ret = luaoc.callStaticMethod("NativeOcClass", "stop", paramtab)
    if not ok then
        local msg = "luaoc error:" .. ret
        print(msg)  
        return 0, msg   
...
function Bridge_ios.getBindData()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"getBindData")
    if not ok then
        return "" --"A501164B366ECFC9E249163873094D50"
    else
        return ret
...
function Bridge_ios.reportRegister()
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"reportRegister")
    if not ok then
        return "" --"A501164B366ECFC9E249163873094D50"
    else
        return ret
